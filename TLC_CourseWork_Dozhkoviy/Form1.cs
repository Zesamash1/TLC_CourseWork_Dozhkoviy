...
namespace TLC_CourseWork_Dozhkoviy
{
    ...
    public partial class TLC : Form
    {
        ...
        private void EditButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Неможливо розпочати редагування\nТаблиця порожня", "Помилка!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ToggleEditMode();
        ...
        private void delButton_Click(object sender, EventArgs e)
        {
            if (dgvAddPassenger.Rows.Count == 0)
            {
                MessageBox.Show("Неможливо розпочати видалення\nТаблиця порожня", "Помилка!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (isRowHeadersVisible)
            {
        ...
        private void saveButton_Click(object sender, EventArgs e)
        {
            if (dgvAddPassenger.Rows.Count == 0)
            {
                MessageBox.Show("Неможливо розпочати збереження файлу\nТаблиця порожня", "Помилка!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void Searchbtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Поле пошуку не може бути порожнім \nВведіть значення", "Помилка!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            // Перевірка на наявність даних у DataGridView
            if (dgvSearchPassenger.Rows.Count == 0)
            {
                MessageBox.Show("Неможливо здійснити пошук\nТаблиця порожня", "Помилка!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void IndividualSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                var maxWeightDestination = FindMaxWeightDestination(passengers, out double TotalWeight);
                ...
                // Показуємо MessageBox з загальною сумою ваги
                MessageBox.Show($"Станція призначення:'{maxWeightDestination}'\nЗагальна вага: {TotalWeight} кг","Станція призначення з найбільшою загальною вагою речей", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        //Методи
        //1. Задаємо потрібні нам методи для таблиці, щоб не було видно стовпець який зазвичай з'являється при запускі програми,
        //та робимо так, щоб користувачу не можна було редагувати таблицю напряму в ній а лише за допомогою текстових полів
        ...
        {
            surnametxt.Text = CapitalizeFirstLetter(surnametxt.Text.Trim());
        ...
        {
            nametxt.Text = CapitalizeFirstLetter(nametxt.Text.Trim());
        ...
        //Валідація імені по-батькові
        private bool ValidatePatronymic()
        {
            patronymictxt.Text = CapitalizeFirstLetter(patronymictxt.Text.Trim());
        ...
        //Валідація станції призначення
        private bool ValidateDestinationStation()
        {
            destinationStationtxt.Text = CapitalizeFirstLetter(destinationStationtxt.Text.Trim());
        ...
        //Валідація загальної ваги речей
        ...
        //Прирівнення значень до текстових полів
        ...
        //Додавання пасажира до таблиці та валідаця на дублікат
        private void AddPassengerToGrid(Passenger passenger)
        {
            foreach (DataGridViewRow row in dgvAddPassenger.Rows)
            {
                if (row.Cells[0].Value == null) continue;
                // Перевірка на унікальність комбінації номер вагона + номер місця
                if (int.Parse(row.Cells[4].Value.ToString()) == passenger.CarriageNumber &&
                    int.Parse(row.Cells[5].Value.ToString()) == passenger.SeatNumber)
                {
                    MessageBox.Show("Це місце вже зайняте\nВведіть інший номер місця", "Увага!", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
        ...
        //метод для видалення обраного рядку
        ...
        //показати повідомлення про вибір
        ...
        private void LoadFile(string filePath)
        {
            DataSet ds = new DataSet();
            ds.ReadXml(filePath);
            // Перевірка, чи присутні всі необхідні таблиці та стовпці
            if (!ds.Tables.Contains("Passenger") ||
                !ds.Tables["Passenger"].Columns.Contains("Прізвище") ||
                !ds.Tables["Passenger"].Columns.Contains("Ім'я") ||
                !ds.Tables["Passenger"].Columns.Contains("По-батькові") ||
                !ds.Tables["Passenger"].Columns.Contains("Станція призначення") ||
                !ds.Tables["Passenger"].Columns.Contains("№ вагона") ||
                !ds.Tables["Passenger"].Columns.Contains("№ місця") ||
                !ds.Tables["Passenger"].Columns.Contains("Кількість речей") ||
                !ds.Tables["Passenger"].Columns.Contains("Загальна вага речей"))
            {
                MessageBox.Show("Файл не підходить для завантаження\nЦе можливо якщо ви спробували завантажити файл з іншою структурою даних", "Помилка!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        //бінарний пошук для пошуку за значенням
        ...
        //Фільтр який приховує значення менше за введене та показує більші
        ...
        //Знаходження станції з найбільшою вагою
        private string FindMaxWeightDestination(List<Passenger> passengers, out double totalWeight)
        {
        ...
        //Оновлюємо таблицю вивівши станцію з найбільшим максимальним значенням
        ...
        //Щоб можна було вийти через звичайний крестик форми додамо даний метод
